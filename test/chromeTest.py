...
def load_page():
    ...
    wd = webdriver.Chrome(chrome_options=options, executable_path=r'C:\Users\Someone\Documents\Browsers\chromedriver.exe')
    wd.maximize_window()
    # load url
    wd.get(url)
...
def jobs_to_dataframe(jobs, webdriver):
    ...
    for item in range(len(jobs)):
        ...
        button_path =  "//section[@class='two-pane-serp-page__detail-view']//button[@class = 'show-more-less-html__button show-more-less-html__button--more']"
        # actions = ActionChains(webdriver)
        # actions.move_to_element(button)
        try:
            WebDriverWait(job, 5).until(expected_conditions.element_to_be_clickable(job.find_element(By.XPATH, button_path))).click()
        # actions.perform()
        # time.sleep(.1)
        except:
            temp_list = job.find_elements(By.XPATH, button_path)
            while len(temp_list) <= 0:
                ...
                time.sleep(.1)
                temp_list
            job.find_element(By.XPATH, button_path).click()
        # finally:
